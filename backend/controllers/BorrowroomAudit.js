import Borrow_room from "../models/BorrowRoomAuditmodel.js";
import Users from "../models/Usermodel.js";
import {Op} from "sequelize";
import Room from "../models/Roombmnmodel.js";
import { SMTPClient } from 'emailjs';
import moment from "moment-timezone";


export const getBorrowroomAudit = async (req, res) => {
    try {
        let response;
        if ((req.role === 1 || req.role === 3 || req.role === 5|| req.role === 9||req.role === 6||req.role === 8) && req.jur_id === 3|| req.jur_id === 5){
...
export const getBorrowroomByIdAudit  = async(req, res) =>{
    try {
        ...
        if (req.role === 3 || req.role === 5 ||req.role === 9 ||req.role === 1||req.role === 8||req.role === 6 ||req.role === 4){
            response = await Borrow_room.findOne({
                attributes:['uuid','nama_kegiatan','nama','ruangan','borrow_date','borrow_time','until_time','until_date','note','status','updateBy', 'note','phone','nama_pengguna','dosen_id','email','jumlah_pengguna','barang'],
                where:{
                    id: borrowroom.id
                },
                include:[{
...
export const createBorrowroomAudit  = async(req, res) =>{
    const {nama_kegiatan,nama, ruangan,dosen_id,jur_id, prodi_id,phone,borrow_date,until_date,borrow_time,until_time,nama_pengguna,email,jumlah_pengguna,barang} = req.body;
    try {
...
export const updateBorrowroomAudit  = async(req, res) =>{
    try {
        const borrowroom = await Borrow_room.findOne({
            where:{
                uuid: req.params.id
        ...
        if(!borrowroom) return res.status(404).json({msg: "Data tidak ditemukan"});
        const {nama_kegiatan,nama, ruangan,dosen_id,jur_id, prodi_id,phone,borrow_date,until_date,borrow_time,until_time,nama_pengguna,status,email,jumlah_pengguna,barang} = req.body;
        if(req.role === 3 || req.role === 5 ||req.role === 9 ||req.role === 1 ||req.role === 8||req.role === 4){
            await Borrow_room.update({nama_kegiatan,nama, ruangan,dosen_id,jur_id, prodi_id,phone,borrow_date,until_date,borrow_time,until_time,status,nama_pengguna,email,jumlah_pengguna,barang,updateBy: req.uuid},{
                where:{
                    id: borrowroom.id
                }
            });
            if (status === 3 && !borrowroom.email_sent) {
            ...
            await Borrow_room.update({nama_kegiatan,nama, ruangan,dosen_id,jur_id, prodi_id,phone,borrow_date,until_date,borrow_time,until_time,nama_pengguna,status,email,jumlah_pengguna,barang,updateBy: req.uuid},{
                where:{
                    [Op.and]:[{id: borrowroom.id}, {userId: req.userId}]
                }
            });
...
export const deleteBorrowroomAudit  = async(req, res) =>{
    try {
        const borrowroom = await Borrow_room.findOne({
            where:{
                uuid: req.params.id
...
export const checkRoomAvailabilityAudit = async (req, res) => {
    try {
