...
const Team = () => {
  ...
  const navigateToEditPage = (uuid) => {
    ...
    navigate(`/daftarpinjamanmb/edit/${uuid}`);
  };

  const navigateToViewPage = (uuid) => {
    // Gantilah '/borrowlist/edit/' sesuai dengan path yang sesuai untuk halaman edit Anda
    navigate(`/daftarpinjamanmb/view/${uuid}`);
  };
  ...
  const getBorrowroomMB = async () => {
    const response = await axios.get("http://localhost:5000/BorrowroomMB");
  ...
  const handleDelete = async (borrowroomId) => {
    await axios.delete(`http://localhost:5000/BorrowroomMB/${borrowroomId}`);
    getBorrowroomMB();
    closeDeleteModal();
  };
  ...
  useEffect(() => {
    axios.get("http://localhost:5000/BorrowroomMB")
      .then(response => {
        setBorrow_room(response.data);
    ...
    <ThemeProvider theme={theme}>
      ...
      <MyProSidebarProvider>
        <div style={{ height: "100%", width: "100%" }}>
          <Helmet>
            <title>SIBor | DaftarPeminjamanMB</title>
          </Helmet>
          <main>
          <Topbar />
    <Box m="20px">
      <Box display="flex" justifyContent="space-between" alignItems="center">
        <Header title="DaftarPinjamanMB " subtitle="Daftar Terbaru" />
        {user && (user.role === 1 || user.role === 2 ) && (
                  <Button href="/daftarpinjamanmb/add" variant="contained" color="success">
                    New
                  </Button>
                     )}
                      {user && (user.role === 1 || user.role === 4 ) && (
                  <Button href="/daftarpinjamanmb/dosen/add" variant="contained" color="success">
                    New
                  </Button>
                     )}
                     {user && (user.role === 1 || user.role === 5 ) && (
                  <Button href="/daftarpinjamanmb/tu/add" variant="contained" color="success">
                    New
                  </Button>
                     )}
      </Box>
